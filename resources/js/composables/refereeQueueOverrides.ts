...
export function persistResultOverridesForSelection(options: OverrideStorageOptions & {
  ...
  const serializedOverrides = JSON.stringify(nextOverrides)
  const writeResult = storageKey
    ? writeSerializedStorageValue(options.storage, storageKey, serializedOverrides)
    : { written: false }
