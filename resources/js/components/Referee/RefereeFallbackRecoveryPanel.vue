<template>
    <button
        ...
        :aria-expanded="
            props.model.isFallbackSetupPanelExpanded ? 'true' : 'false'
        "
    >
        <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
                <div
                    class="text-[11px] font-black tracking-[0.22em] text-sky-300 uppercase"
                >
                    Manual Recovery Setup
                </div>
                <span
                    class="inline-flex rounded-full border px-3 py-1 text-[10px] font-black tracking-[0.18em] uppercase"
                    :class="props.model.fallbackSetupStatusToneClass"
                >
                    {{ props.model.fallbackSetupStatusLabel }}
                </span>
                <span
                    v-if="props.model.setupSource === 'assigned_setup'"
                    class="inline-flex rounded-full border border-yellow-400/30 bg-yellow-500/10 px-3 py-1 text-[10px] font-black tracking-[0.18em] text-yellow-100 uppercase"
                >
                    Non-authoritative
                </span>
            </div>
            <div
                class="mt-2 text-sm font-black tracking-[0.14em] text-white uppercase"
            >
                Temporary recovery values
            </div>
            <div class="mt-2 text-xs text-slate-400">
                Used only when Event Host assignment is missing or snapshots
                need recovery.
            </div>
        </div>
        <div class="flex shrink-0 items-center gap-2">
            <div
                class="hidden flex-wrap items-center justify-end gap-2 xl:flex"
            >
                <span
                    class="inline-flex max-w-52 truncate rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] font-black tracking-[0.16em] text-slate-200 uppercase"
                >
                    {{ props.model.fallbackSetupHostSummaryLabel }}
                </span>
                <span
                    class="inline-flex max-w-56 truncate rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] font-black tracking-[0.16em] text-slate-200 uppercase"
                >
                    {{ props.model.fallbackTournamentSummaryLabel }}
                </span>
                <span
                    class="inline-flex rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] font-black tracking-[0.16em] text-slate-200 uppercase"
                >
            ...
            >
                <ChevronDownIcon
                    :class="`h-5 w-5 text-slate-300 transition-transform duration-300 ${props.model.isFallbackSetupPanelExpanded ? 'rotate-180' : ''}`"
                />
            </div>
        </div>
    </button>
    ...
    <div class="mt-4 flex flex-wrap gap-2 xl:hidden">
        <span
            class="inline-flex max-w-full truncate rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] font-black tracking-[0.16em] text-slate-200 uppercase"
        >
            {{ props.model.fallbackSetupHostSummaryLabel }}
        </span>
        <span
    ...
    <div
        v-if="props.model.isFallbackSetupPanelExpanded"
        class="mt-4 border-t border-white/8 pt-4"
    >
        <div class="mb-4 text-xs text-slate-400">
            {{
                props.model.isAdminRecoveryLocked
                    ? 'Event Host assignment is active. Manual recovery values are locked until the Event Host or assignment is unavailable.'
                    : 'Choose a temporary tournament and gilam only for recovery.'
            }}
        </div>
        ...
        <div class="grid gap-4">
                ...
                <div>
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger as-child>
                            <button
                                :disabled="props.model.isAdminRecoveryLocked"
                                class="flex h-11 w-full items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 text-left text-sm font-bold text-white transition-all hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-50"
                            >
                        ...
                        <DropdownMenuContent
                            class="w-(--reka-dropdown-menu-trigger-width) min-w-(--reka-dropdown-menu-trigger-width) rounded-2xl border border-white/10 bg-[#0f172a] p-2 shadow-2xl"
                        >
                            <DropdownMenuItem
                            ...
                            >
                                ...
                                <span class="text-xs font-black tracking-wider"
                                    >Select Tournament</span
                                >
                            </DropdownMenuItem>
                            <div
                                class="custom-scrollbar max-h-64 overflow-y-auto"
                            >
                                <DropdownMenuItem
                                    v-for="tournament in props.model
                                        .tournaments"
                                    :key="tournament.id"
                                    @select="
                                        props.actions.selectTournament(
                                            tournament.id,
                                        )
                                    "
                                    class="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-gray-300 transition-all hover:bg-blue-500/20 hover:text-blue-400"
                                >
                                    ...
                                    <span
                                        class="truncate text-xs font-black tracking-wider"
                                        >{{ tournament.name
                                        }}{{
                                            tournament.saved ? ' *' : ''
                                        }}</span
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger as-child>
                            ...
                            >
                                <span class="flex min-w-0 items-center gap-2">
                                    ...
                                    />
                                    <span class="truncate">{{
                                        props.model.manualSelectedRing
                                            ? `Gilam ${props.model.manualSelectedRing}`
                                            : 'Choose a gilam'
                                    }}</span>
                                </span>
                                <ChevronDownIcon
                        ...
                        >
                            <DropdownMenuItem
                                ...
                                :key="ringOption"
                                @select="
                                    props.actions.selectRing(String(ringOption))
                                "
                                class="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-gray-300 transition-all hover:bg-blue-500/20 hover:text-blue-400"
                            >
                                <span
            ...
            <div class="flex flex-wrap gap-2 pt-1">
                <button
                    ...
                    :disabled="
                        !props.model.syncHasServer ||
                        props.model.isCheckingStatus
                    "
                    class="h-10 rounded-2xl border border-blue-500/30 bg-blue-500/10 px-4 text-xs font-black tracking-[0.18em] text-blue-100 uppercase transition-all hover:bg-blue-500/20 disabled:opacity-50"
                >
                    Test Host
                </button>
                <button
                    ...
                    :disabled="
                        !props.model.syncHasServer ||
                        props.model.isFetchingAll ||
                        props.model.isLoadingTournaments
                    "
                    class="inline-flex h-10 items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 text-xs font-black tracking-[0.18em] text-white uppercase transition-all hover:bg-white/10 disabled:opacity-50"
                >
                    <Upload
                        :class="[
                            'h-4 w-4',
                            props.model.isFetchingAll ? 'animate-pulse' : '',
                        ]"
                    />
                    Refresh Tournaments
                </button>
                <button
                    ...
                    :disabled="
                        props.model.isLoadingMatches ||
                        props.model.isLoadingTournaments ||
                        props.model.isCheckingStatus ||
                        props.model.isLiveSnapshotRecoveryBusy
                    "
