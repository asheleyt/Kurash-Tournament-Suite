<template>
  ...
  >
    <div class="min-w-0 flex-1">
      <div class="flex flex-wrap items-center gap-2">
        <div class="text-[11px] font-black uppercase tracking-[0.22em] text-sky-300">Admin Recovery Setup</div>
      ...
      <div class="mt-2 text-sm font-black uppercase tracking-[0.14em] text-white">Temporary Admin recovery values</div>
      <div class="mt-2 text-xs text-slate-400">
        Used only when Admin assignment is missing or snapshots need recovery.
      </div>
    </div>
  ...
  <div v-if="props.model.isFallbackSetupPanelExpanded" class="mt-4 border-t border-white/8 pt-4">
    <div class="mb-4 text-xs text-slate-400">
      {{ props.model.isAdminRecoveryLocked ? 'Admin-assigned setup is active. Manual recovery values are locked until the Event Host or assignment is unavailable.' : 'Choose a temporary tournament and gilam only for Admin recovery.' }}
    </div>

    <div class="grid gap-4">
      <div class="rounded-2xl border border-white/8 bg-white/3 px-4 py-3">
        <div class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Event Host</div>
        <div class="mt-1 text-sm font-black text-white">
          {{ props.model.syncHasServer ? props.model.fallbackSetupHostSummaryLabel : 'Set in Controller Pairing > Advanced' }}
        </div>
        <div class="mt-1 text-[11px] font-semibold leading-5 text-slate-400">
          Admin Recovery uses the same Event Host as Controller Pairing.
        </div>
      </div>
