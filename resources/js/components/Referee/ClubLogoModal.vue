...
<template>
<template>
  <div ref="rootContainer" class="h-screen bg-[#1a1f2e] p-4 flex flex-col" :class="isSettingsOpen ? 'overflow-y-auto justify-start' : 'overflow-hidden justify-center'">
    <div class="w-full max-w-400 mx-auto flex flex-col gap-4">
      <!-- Settings Panel -->
        <div class="bg-black/20 backdrop-blur-md border border-white/5 shadow-xl rounded-2xl p-4">
          <button 
            @click="isSettingsOpen = !isSettingsOpen"
            class="w-full flex items-center justify-between text-white font-bold tracking-wide focus:outline-none"
          >
            <div class="flex items-center gap-2">
              <SettingsIcon class="h-6 w-6 text-gray-400" />
              <span>GILAM CONTROLLER PANEL</span>
            </div>
            <ChevronDownIcon :class="`h-6 w-6 transition-transform duration-300 ${isSettingsOpen ? 'rotate-180' : ''}`" />
          </button>
          
          <div v-show="isSettingsOpen" class="mt-4 border-t border-gray-700 pt-4">
            <!-- Settings Tabs -->
            <div class="flex gap-4 mb-6 border-b border-gray-700 pb-2">
              <button 
                @click="settingsTab = 'matchlist'"
                class="px-4 py-2 text-sm font-bold uppercase tracking-wider transition-colors relative"
            ...
            <div v-if="settingsTab === 'matchlist'" class="w-full flex flex-col gap-6 animate-in fade-in slide-in-from-left-2 duration-150">
              ...
              <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div class="flex items-center gap-2 flex-wrap">
                  <span :class="isLoadingTournaments ? 'text-blue-400' : (isOnline ? 'text-green-400' : 'text-red-400')">●</span>
                  <span 
                    class="text-xs font-black uppercase tracking-wider"
                    :class="isLoadingTournaments ? 'text-blue-400' : (isOnline ? 'text-green-400' : 'text-red-400')"
                  >
                    {{ isLoadingTournaments ? 'Syncing' : (isOnline ? 'Online' : 'Offline') }}
                  </span>
                  <span
                    v-if="isOnline && !isLoadingTournaments && (!tournaments || !tournaments.length)"
                    class="ml-2 px-2 py-0.5 rounded bg-white/5 border border-white/10 text-gray-300 text-[10px] font-bold uppercase tracking-wider"
                  >
                    No Tournaments
                  </span>
                  <span v-if="isLocalData" class="ml-2 px-2 py-0.5 rounded bg-yellow-500/20 border border-yellow-500/40 text-yellow-300 text-[10px] font-bold uppercase tracking-wider">Local Cache</span>
                  <button 
                    @click="loadTournaments" 
                    :disabled="isLoadingTournaments"
                    class="ml-3 h-7 w-7 rounded-lg font-bold uppercase tracking-wider transition-all flex items-center justify-center bg-white/5 hover:bg-white/10 text-white border border-white/10 disabled:opacity-50"
                    title="Refresh tournaments"
                  >
                    <RefreshCw :class="['w-4 h-4', isLoadingTournaments ? 'animate-spin' : '']" />
                  </button>
                  <button
                    @click="fetchAllTournaments"
                    :disabled="isFetchingAll || isLoadingTournaments"
                    class="ml-2 h-7 px-2 rounded-lg font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 text-white border border-white/10 disabled:opacity-50"
                    title="Fetch from Kurash System"
                  >
                    <Upload :class="['w-4 h-4', isFetchingAll ? 'animate-pulse' : '']" />
                    <span class="hidden sm:inline text-[10px]">Fetch Tournament</span>
                  </button>
                </div>
                <div class="flex items-center gap-3 flex-wrap w-full md:w-auto">
                  <input
                    v-model="adminBase"
                    @change="persistAdminBase"
                    placeholder="Admin API e.g. https://admin.example.com/api"
                    class="h-9 w-full md:w-80 bg-white/5 border border-white/10 rounded-xl px-3 text-white placeholder-gray-500 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none font-bold"
                    title="Admin API Base"
                  />
                  <DropdownMenu>
                    <DropdownMenuTrigger as-child>
                      <button class="h-10 px-4 rounded-3xl bg-white/5 border border-white/10 text-white font-bold flex items-center gap-2 hover:bg-white/10 w-full md:w-auto justify-between">
                    ...
                    <DropdownMenuContent class="w-80 bg-[#0f172a] border-white/10 p-2 shadow-2xl rounded-2xl">
                      <DropdownMenuItem
                        @select="selectedTournamentId = null"
                      ...
                      >
                        <span class="w-4 h-4 inline-flex items-center justify-center">
                          <CheckCircle2 v-if="selectedTournamentId === null" class="w-4 h-4 text-blue-400" />
                        </span>
                      ...
                      <div class="max-h-64 overflow-y-auto custom-scrollbar">
                        <DropdownMenuItem
                          ...
                          :key="t.id"
                          @select="selectedTournamentId = t.id"
                          class="flex items-center gap-3 px-3 py-2 rounded-xl cursor-pointer hover:bg-blue-500/20 text-gray-300 hover:text-blue-400 transition-all"
                        >
                          <span class="w-4 h-4 inline-flex items-center justify-center">
                            <CheckCircle2 v-if="selectedTournamentId === t.id" class="w-4 h-4 text-blue-400" />
                          </span>
                          <span class="text-xs font-black tracking-wider truncate">{{ t.name }}{{ t.saved ? ' 🛢' : '' }}</span>
                        </DropdownMenuItem>
                      </div>
                    </DropdownMenuContent>
                  </DropdownMenu>
                  <div class="relative inline-flex items-center w-full md:w-auto">
                    <DropdownMenu>
                      <DropdownMenuTrigger as-child>
                        <button class="h-10 px-4 rounded-3xl bg-white/5 border border-white/10 text-white font-bold flex items-center gap-2 hover:bg-white/10 disabled:opacity-50 w-full md:w-auto justify-between"
                          :disabled="isLoadingBrackets || !bracketOptions.length"
                          title="Select Bracket"
                        >
                          <Search class="w-4 h-4 text-gray-400" />
                          <span class="truncate max-w-40">
                            {{ (bracketOptions.find(b => String(b.id) === String(selectedBracketId))?.name) || 'Bracket' }}
                          </span>
                          <ChevronDownIcon class="w-4 h-4 text-gray-400" />
                        </button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent class="w-64 bg-[#0f172a] border-white/10 p-2 shadow-2xl rounded-2xl">
                        <DropdownMenuItem
                          @select="selectedBracketId = null"
                          class="flex items-center gap-3 px-3 py-2 rounded-xl cursor-pointer hover:bg-blue-500/20 text-gray-300 hover:text-blue-400 transition-all"
                        >
                          <span class="w-4 h-4 inline-flex items-center justify-center">
                            <CheckCircle2 v-if="selectedBracketId === null" class="w-4 h-4 text-blue-400" />
                          </span>
                          <span class="text-xs font-black tracking-wider">Bracket</span>
                        </DropdownMenuItem>
                        <div class="max-h-60 overflow-y-auto custom-scrollbar">
                          <DropdownMenuItem
                            v-for="b in bracketOptions"
                            :key="b.id"
                            @select="selectedBracketId = b.id"
                            class="flex items-center gap-3 px-3 py-2 rounded-xl cursor-pointer hover:bg-blue-500/20 text-gray-300 hover:text-blue-400 transition-all"
                          >
                            <span class="w-4 h-4 inline-flex items-center justify-center">
                              <CheckCircle2 v-if="String(selectedBracketId) === String(b.id)" class="w-4 h-4 text-blue-400" />
                            </span>
                            <span class="text-xs font-black tracking-wider truncate">{{ b.name }}</span>
                          </DropdownMenuItem>
                        </div>
                      </DropdownMenuContent>
                    </DropdownMenu>
                    <div v-if="isLoadingBrackets" class="ml-2 flex items-center gap-1 text-blue-400">
                      <RefreshCw class="w-4 h-4 animate-spin" />
                      <span class="text-[10px] font-black uppercase tracking-wider">Loading</span>
                    </div>
                    <div v-if="isUpdatingMatches" class="ml-2 flex items-center gap-1 text-blue-400">
                      <RefreshCw class="w-4 h-4 animate-spin" />
                      <span class="text-[10px] font-black uppercase tracking-wider">Updating</span>
                    </div>
                  </div>
                  <button 
                    @click="syncTournament" 
                    :disabled="!selectedTournamentId || isSyncing"
                    class="h-9 px-4 rounded-xl font-bold uppercase tracking-wider transition-all w-full md:w-auto"
                    :class="isSyncing ? 'bg-gray-700 text-gray-400' : 'bg-blue-600 hover:bg-blue-500 text-white'"
                  >
                    Load Matches
                  </button>
                <button
                  @click="openClubLogoModal"
                  class="h-9 px-3 rounded-xl font-bold uppercase tracking-wider transition-all bg-emerald-700 hover:bg-emerald-600 text-white w-full md:w-auto"
                >
                  Club Logo
                </button>
                </div>
              ...
              <div class="bg-black/30 rounded-2xl p-4 border border-white/10">
                <div class="mb-2">
                  <span
                    class="px-2 py-0.5 rounded bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-wider"
                    :class="bracketFormatClass"
                  >
                    {{ bracketFormatLabel }}
                  </span>
                </div>
                  ...
                  <table class="min-w-full text-sm text-left">
                    <thead class="text-gray-400">
                      <tr>
                        ...
                        <th class="px-3 py-2">Bracket</th>
                        <th class="px-3 py-2">Round</th>
                        <th class="px-3 py-2">Stage</th>
                        <th class="px-3 py-2">Player Blue</th>
                        <th class="px-3 py-2">Player Green</th>
                    ...
                    <tbody>
                      <tr 
                        v-for="m in matchesList" 
                        :key="m.id ?? m.remote_id" 
                        class="border-t border-white/5"
                        :class="(nextUpcomingMatchId === (m.id ?? m.remote_id)) ? 'bg-blue-500/10' : ''"
                      >
                        <td class="px-3 py-2 text-white font-bold">{{ m.remote_id ?? m.id }}</td>
                        <td class="px-3 py-2 text-gray-300">{{ m._bracketLabel ?? getBracketLabel(m) }}</td>
                        <td class="px-3 py-2 text-gray-300">{{ m.round_number ?? m.round }}</td>
                        <td class="px-3 py-2 text-gray-300">{{ getStageLabel(m) }}</td>
                        <td class="px-3 py-2 text-blue-400 font-bold">
                          {{ getDisplayName(m, 'blue') }}
                        </td>
                        <td class="px-3 py-2 text-green-400 font-bold">
                          {{ getDisplayName(m, 'green') }}
                        </td>
                        <td class="px-3 py-2">
                          <div class="flex items-center gap-2">
                            <RefreshCw 
                              v-if="isUpdatingMatches && (updatingMatchId === (m.remote_id ?? m.id))" 
                              class="w-4 h-4 animate-spin text-blue-400"
                            />
                            <span 
                              class="px-2 py-1 rounded text-xs font-bold"
                              :class="m.status === 'completed' ? 'bg-green-700 text-white' : (m.status === 'current' ? 'bg-yellow-700 text-white' : 'bg-gray-700 text-gray-300')"
                            >
                              {{ m.status }}
                            </span>
                          </div>
      ...
      <div v-if="settingsTab === 'match'" class="w-full flex flex-col gap-6 animate-in fade-in slide-in-from-left-2 duration-150">
                                    ...
                                    <DropdownMenuContent class="w-64 bg-[#0f172a] border-white/10 p-2 shadow-2xl">
                                        ...
                                        <div class="px-2 pb-2">
                                            <Input 
                                                ...
                                                class="h-9 bg-black/40 border-white/10 text-white placeholder:text-gray-700 focus:ring-blue-500/20"
                                                @click.stop
                                            />
                                        </div>
                                        ...
                                        <div class="max-h-75 overflow-y-auto custom-scrollbar mt-1">
                                            ...
                                            >
                                                <div class="w-8 h-5 rounded overflow-hidden border border-white/10">
                                                    <img :src="`/images/${availableFlags[country.code]}`" class="w-full h-full object-cover" />
                                                </div>
                                    ...
                                    <DropdownMenuContent class="w-64 bg-[#0f172a] border-white/10 p-2 shadow-2xl">
                                        ...
                                        <div class="px-2 pb-2">
                                            <Input 
                                                ...
                                                placeholder="Search country..."
                                                class="h-9 bg-black/40 border-white/10 text-white placeholder:text-gray-700 focus:ring-green-500/20"
                                                @click.stop
                                            />
                                        </div>
                                        <DropdownMenuSeparator class="bg-white/5" />
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-black/40 rounded-2xl p-6 border border-white/10">
                ...
                <input
                    ...
                    class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all font-bold"
                />
                <div class="mt-2 text-[11px] text-gray-500 font-semibold">Optional for offline/manual matches</div>
            </div>

            <div class="bg-black/40 rounded-2xl p-6 border border-white/10">
                <label class="block text-[10px] text-gray-500 font-black uppercase tracking-widest mb-4">Bracket Category</label>
                <input
                    v-model="tempSettings.bracketCategory"
                    type="text"
                    placeholder="e.g. U18"
                    class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:border-green-500 focus:ring-2 focus:ring-green-500/20 outline-none transition-all font-bold"
                />
        ...
        <div class="grid grid-cols-1">
            <div class="flex justify-center py-4">
                <button 
                    ...
                    class="group relative px-16 py-6 bg-green-600 hover:bg-green-500 text-white font-black text-xl rounded-2xl shadow-[0_10px_40px_rgba(22,163,74,0.3)] transition-all active:scale-95 flex items-center gap-4 border-b-4 border-green-800 hover:border-green-700 uppercase tracking-[0.2em] overflow-hidden"
                >
      ...
      <div v-else-if="settingsTab === 'keyboard'" class="animate-in fade-in slide-in-from-right-2 duration-150">
        <KeyboardSettings 
          :bindings="bindings"
        ...
        <!-- Timer Display -->
        <div class="bg-black/20 backdrop-blur-md border border-white/5 shadow-xl rounded-2xl p-4 sm:p-8 relative">
          <div class="w-full flex flex-col lg:flex-row items-center justify-between gap-6">
            <!-- Left group: Match ID + Category (same row) -->
            ...
            <!-- Right group: Gender + Weight Division (same row) -->
            <div class="flex items-center gap-6 shrink-0">
              <div class="bg-white/10 backdrop-blur rounded-2xl border border-white/10 py-3 px-6 w-44 sm:w-48 md:w-56 text-center">
            ...
            <div class="flex gap-2 mb-4">
              <Dialog v-model:open="isAdjustTimeOpen">
                ...
                <DialogContent class="bg-[#1e293b] border-gray-700">
                  <DialogHeader>
                    <DialogTitle class="text-white">Adjust Current Time</DialogTitle>
                    <DialogDescription class="text-gray-400">Modify the current timer value.</DialogDescription>
                  </DialogHeader>
                  <div class="flex gap-4 justify-center py-4">
                    <div class="flex flex-col items-center">
                      <label class="text-xs text-gray-500 mb-1">Minutes</label>
                      <input v-model="adjustMinutes" type="number" min="0" class="w-20 bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white text-center text-xl font-mono focus:border-blue-500 outline-none" />
                    </div>
                    <div class="flex items-center text-white text-xl font-bold pt-4">:</div>
                    <div class="flex flex-col items-center">
                      <label class="text-xs text-gray-500 mb-1">Seconds</label>
                      <input v-model="adjustSeconds" type="number" min="0" max="59" class="w-20 bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white text-center text-xl font-mono focus:border-blue-500 outline-none" />
                    </div>
                  </div>
                  <DialogFooter>
              ...
              <Dialog v-model:open="isSetStartTimeOpen">
                ...
                <DialogContent class="bg-[#1e293b] border-gray-700">
                  <DialogHeader>
                    <DialogTitle class="text-white">Set Start Time</DialogTitle>
                    <DialogDescription class="text-gray-400">Set a new starting time for the match.</DialogDescription>
                  </DialogHeader>
                  <div class="flex gap-4 justify-center py-4">
                    <div class="flex flex-col items-center">
                      <label class="text-xs text-gray-500 mb-1">Minutes</label>
                      <input v-model="startMinutes" type="number" min="0" class="w-20 bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white text-center text-xl font-mono focus:border-blue-500 outline-none" />
                    </div>
                    <div class="flex items-center text-white text-xl font-bold pt-4">:</div>
                    <div class="flex flex-col items-center">
                      <label class="text-xs text-gray-500 mb-1">Seconds</label>
                      <input v-model="startSeconds" type="number" min="0" max="59" class="w-20 bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white text-center text-xl font-mono focus:border-blue-500 outline-none" />
                    </div>
                  </div>
                  <DialogFooter>
                ...
                <DialogContent class="bg-[#1e293b] border-gray-700">
                  <DialogHeader>
                    ...
                    <DialogDescription class="text-gray-400">
                      Are you sure you want to reset the timer to default?
                    </DialogDescription>
                  </DialogHeader>
                  <DialogFooter>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="flex gap-2">
                ...
                <Dialog v-model:open="isSetBreakTimeOpen">
                  ...
                  <DialogContent class="bg-[#1e293b] border-gray-700">
                    <DialogHeader>
                      <DialogTitle class="text-white">Set Break Time</DialogTitle>
                      <DialogDescription class="text-gray-400">
                        Set the duration for the break.
                      </DialogDescription>
                    </DialogHeader>
                    ...
                    <div class="flex items-center justify-center gap-4 py-6">
                      <div class="flex flex-col items-center">
                        ...
                        <input
                          ...
                          min="0"
                          max="60"
                          class="w-20 h-16 bg-gray-900 border border-gray-700 rounded-xl text-3xl font-mono text-center text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none"
                        />
                      </div>
              ...
              <Dialog v-model:open="isResetMatchOpen">
                ...
                <DialogContent class="bg-[#1e293b] border-gray-700">
                  <DialogHeader>
                    ...
                    <DialogDescription class="text-gray-400">
                      Are you sure you want to reset the ENTIRE match? This cannot be undone.
                    </DialogDescription>
                  </DialogHeader>
                  <DialogFooter>
                    <DialogClose as-child>
    ...
    <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
