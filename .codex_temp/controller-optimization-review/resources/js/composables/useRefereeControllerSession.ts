...
export function useRefereeControllerSession(options: UseRefereeControllerSessionOptions) {
  ...
  async function refreshAssignedSetupState(showErrorBanner = false) {
  ...
  async function reconnectKnownDeviceSession(showSuccessBanner = false) {
    ...
    try {
      ...
      await refreshAssignedSetupState(false)
      if (options.shouldAttemptLiveSnapshotRecovery()) {
        void options.attemptLiveSnapshotRecovery({ skipOnlineCheck: true })
      }
      updatePairingStatusDetail(null)
      if (showSuccessBanner) options.showBanner('Known device reconnected to the Admin Host.', 'success', 2500)
      return true
    } catch (error: any) {
      ...
      pairingState.value = 'paired_known_device'
      updatePairingStatusDetail(error?.message || 'Known device reconnect failed. Manual fallback remains available while the local LAN connection is restored.')
      return false
    } finally {
